...
- **The test behind an acceptance criterion must fail when the feature is removed.** A green test is not evidence — check what it would take to break it. Watch for one-sided bounds (`expect(n).toBeLessThan(150)` passes at 0), assertions on a state the test never actually entered, and tests named for a case their setup skips. Found in #26 (the 5,000-element virtualisation test renders 0 rows and passes) and #25 (the "clears the query on reopen" test closes the palette first, so it never covers ⌘K-while-open).
